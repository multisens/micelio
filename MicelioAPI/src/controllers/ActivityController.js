...
class ActivityController {
  ...
  async index(request, response) {
    ...
    const session = await knex('Session').where({session_id: session_id, game_id: game_id}).first()
    if (!session) {
    ...
    const activities = await knex('Activity')
        .select('Activity.*')
        .select('entities.entity_id', 'entities.properties as entity_properties',
            'entities.role as entity_role', 'entities.position_x as entity_position_x',
            'entities.position_y as entity_position_y','entities.position_z as entity_position_z')
        .select('agents.agent_id', 'agents.properties as agent_properties', 'agents.role as agent_role',
            'agents.position_x as agent_position_x', 'agents.position_y as agent_position_y',
            'agents.position_z as agent_position_z')
        .select('entity.name as entity_name')
        .select('agent.name as agent_name')
        .leftJoin('ActivityAgents as agents', 'agents.activity_id', '=', 'Activity.activity_id')
    ...
    activities.forEach(activity => {
      if(!activityStruct[activity.activity_id]) {
        const {activity_id, name, time, influenced_by, position_x, position_y, properties} = activity
        activityStruct[activity.activity_id] = {
          activity_id,
          name,
          time,
          influenced_by,
      ...
      if(activity.entity_id) {
        const {entity_id, entity_name: name, entity_role: role, entity_position_x: position_x,
          entity_position_y: position_y, entity_position_z: position_z, entity_properties: properties} = activity

        activityStruct[activity.activity_id].entities.push({
          entity_id,
          name,
          role,
          position_x,
      ...
      if(activity.agent_id) {
        const {agent_id, agent_name: name, agent_role: role, agent_position_x: position_x,
          agent_position_y: position_y, agent_position_z: position_z, agent_properties: properties} = activity

        activityStruct[activity.activity_id].agents.push({
          agent_id,
          name,
          role,
          position_x,
    ...
    session.activities = Object.values(activityStruct)
    return response.json(session)

    /*const baseDir = path.join(__dirname, '..', '..', '..', 'Scripts', 'MicelioParser', 'Control Harvest', 'JsonConverter', 'Exports', 'sessao208.json')

    if(Math.round((Math.random()) % 2) === 0) {
      return response.json(session)
    }else {
      return response.json(JSON.parse(fs.readFileSync(baseDir).toString()))
    }*/
  ...
  async create(request, response) {
    ...
    try {
      ...
      registered_agents = registered_agents.map((row) => {
        return row.agent_id
      });
      registered_entities = registered_entities.map((row) => {
